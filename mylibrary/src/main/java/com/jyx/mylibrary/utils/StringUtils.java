...
/**
 * @author jyx
 * @ctime 2017/10/24:14:39
 * @explain 字符串工具类，提供一些字符串相关的便捷方法
 */
...
public class StringUtils {
    ...
    public static boolean isByteEmpty(byte[] arr) {
        if (arr != null && new String(arr).trim().length() > 0) {
            return false;
        } else {
            return true;
        }
    ...
    public static Boolean isObjectEmpty(Object object) {
        if (object instanceof String) {
            ...
            return isByteEmpty((byte[]) object);
        } else {
            if (object == null) {
                return true;
    ...
    public static String changeDistance(String distance) {
        if (StringUtils.isObjectEmpty(distance)) {
            return "0.00 km";
        } else {
            return Float.valueOf(distance) / 1000 + "km";
        }
    }
