...
public class NineGridLayout extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (showNumber == 1) {
            mSingleWidth = mTotalWidth;
        } else if (showNumber == 2) {
            mSingleWidth = (int) ((mTotalWidth - mSpacing * (3 - 1)) / 2);
        } else {
    ...
    private void layoutParams() {
        ...
        //根据子view数量确定高度
        if (showNumber == 2) {
            params.height = displaySize.x / 3 - (int) DEFUALT_SPACING;
        } else {
            params.height = (displaySize.x / 3 - (int) DEFUALT_SPACING) * 12 / 17;
        }
...
//        params.height = (int) (singleHeight * mRows + mSpacing * (mRows - 1));
    ...
    private void layoutImageView(ImageView imageView, int i, String String, boolean showNumFlag) {
        int singleWidth = (int) ((mTotalWidth - mSpacing * (3 - 1)) / 3);
        if (showNumber == 2) {
