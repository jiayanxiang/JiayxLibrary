...
public class MyRecyclerView extends RecyclerView {
    ...
    private MyRecyclerViewAdapter lfAdapter;
    private boolean isLoadMore;
    private boolean isRefresh = true;
    private OnItemClickListener itemListener;
    private float mLastY;//上一次Y 值
    public MyRecyclerViewFooter recyclerViewFooter;
    private MyRecyclerViewHeader recyclerViewHeader;
    ...
    private OnPullOrRefreshListener onPullOrRefreshListener;
    ...
    public void setAdapter(Adapter adapter) {
        ...
        lfAdapter = new MyRecyclerViewAdapter(getContext(), adapter);
    ...
    private void updateHeaderHeight(float delta) {
        ...
        if (isRefresh && !mPullRefreshing) { // 未处于刷新状态，更新箭头
            if (recyclerViewHeader.getVisiableHeight() > mHeaderViewHeight) {
                recyclerViewHeader.setState(MyRecyclerViewHeader.STATE_READY);
            } else {
                recyclerViewHeader.setState(MyRecyclerViewHeader.STATE_NORMAL);
            }
        }
    }
...
//        lfAdapter.notifyDataSetChanged();
        if (mPullRefreshing) {
            long de = 1000;
            if (isSuccess) {
                recyclerViewHeader.setState(MyRecyclerViewHeader.STATE_SUCCESS);
            } else {
                recyclerViewHeader.setState(MyRecyclerViewHeader.STATE_FRESH_FAILT);
//                recyclerViewHeader.setMessage(message);
    ...
    private void updateFooterHeight(float delta) {
        ...
        if (isLoadMore) {
            if (height > PULL_LOAD_MORE_DELTA) {
                recyclerViewFooter.setState(MyRecyclerViewFooter.STATE_READY);
                mPullLoading = true;
            } else {
                recyclerViewFooter.setState(MyRecyclerViewFooter.STATE_NORMAL);
                mPullLoading = false;
                mPullLoad = false;
            }
        }
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (!mPullRefreshing && layoutManager.findFirstVisibleItemPosition() == 0) {
                    ...
                    if (isRefresh
                        ...
                        recyclerViewHeader.setState(MyRecyclerViewHeader.STATE_REFRESHING);
                        if (onPullOrRefreshListener != null) {
                            onPullOrRefreshListener.onRefresh();
                        }
                    }
                ...
                if (isLoadMore && mPullLoading && layoutManager.findLastVisibleItemPosition() == lfAdapter.getItemCount() - 1
                    ...
                    recyclerViewFooter.setState(MyRecyclerViewFooter.STATE_LOADING);
                    mPullLoad = true;
                    startLoadMore();
                }
                resetHeaderHeight();
    ...
    private void startLoadMore() {
        if (onPullOrRefreshListener != null) {
            recyclerViewFooter.setState(MyRecyclerViewFooter.STATE_LOADING);
            onPullOrRefreshListener.onLoadMore();
        }
    }
...
//        lfAdapter.notifyDataSetChanged();
    ...
    private void initWithContext(Context context) {
        ...
        recyclerViewHeader = new MyRecyclerViewHeader(context);
        recyclerViewFooter = new MyRecyclerViewFooter(context);
...
//            super.onItemRangeMoved(fromPosition + lfAdapter.getheaderViewCount(), toPosition + lfAdapter.getheaderViewCount(), itemCount);
    ...
    public void setOnPullOrRefreshListener(OnPullOrRefreshListener onPullOrRefreshListener) {
        this.onPullOrRefreshListener = onPullOrRefreshListener;
    }
