...
public class NodeProgressView extends View {
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        mNodeProgressColor = mTypedArray.getColor(R.styleable.nodeProgress_node_progresscolor, ValuesGainUtil.getInstance().getValuesColor(getContext(), R.color.greenery_color));

        mTextColor = mTypedArray.getColor(R.styleable.nodeProgress_node_tip, Color.GRAY);
        mTextProgressColor = mTypedArray.getColor(R.styleable.nodeProgress_node_progress_tip, ValuesGainUtil.getInstance().getValuesColor(getContext(), R.color.greenery_color));

        mLineColor = mTypedArray.getColor(R.styleable.nodeProgress_node_bar, Color.GRAY);
        mLineProgressColor = mTypedArray.getColor(R.styleable.nodeProgress_node_progress_bar, ValuesGainUtil.getInstance().getValuesColor(getContext(), R.color.greenery_color));
    ...
    private void init() {
        mNodeRadio = DisplayUtils.dip2px(getContext(), 10);
        ...
        mTextPaint.setTextSize(DisplayUtils.dip2px(getContext(), 12));
        ...
        mTextProgressPaint.setTextSize(DisplayUtils.dip2px(getContext(), 12));


        mLinePaint = new Paint();
        mLinePaint.setAntiAlias(true);
        mLinePaint.setStrokeWidth(DisplayUtils.dip2px(getContext(), 2));
        mLinePaint.setStyle(Paint.Style.FILL);
        mLinePaint.setColor(mLineColor);

        mLineProgressPaint = new Paint();
        mLineProgressPaint.setAntiAlias(true);
        mLineProgressPaint.setStrokeWidth(DisplayUtils.dip2px(getContext(), 2));
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            if (mCurentNode >= i) {
                ...
                if (mNodeBitmap != null) {
                    ...
                    canvas.drawCircle(point.x, point.y, 10, mNodePaint);
                }
    ...
    private void drawTextProgress(Canvas canvas) {
        for (int i = 0; i < nodes.size(); i++) {
            ...
            if (mCurentNode >= i) {
                ...
                canvas.drawText(text, point.x+getPaddingLeft(), point.y + mNodeRadio + fmi.bottom - fmi.top, mTextPaint);
            }
